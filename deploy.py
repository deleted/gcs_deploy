...
def work(path, dest_bucket, start_path):
...
def sync_parallel(filename_iterator, options):
    ...
    #work(filename_iterator.next(), options.dest_bucket, options.source_dir) #TMP
    pool = Pool(2, initargs=(options.inventory_db,))
    results = set()
    for filename in filename_iterator:
        logger.debug("Adding " + filename)
        results.add(pool.apply_async(work, (filename, options.dest_bucket, options.source_dir), callback=lambda result, path=filename, results=results: mark_complete(path)))
    logger.debug("initial results: %d" % len(results))
    pool.close()
    pool.join()

def names_from_db(dbname, which="untransfered"):
    global connection
